...
export function SpinningWheel({ winner, onWinnerSelected }: SpinningWheelProps) {
  ...
  const spinWheel = () => {
    ...
    const songs: ("A" | "B" | "C")[] = ["A", "B", "C"];
    const winnerIndex = songs.indexOf(winner);

    // Calculate final rotation
    // Each section is 120 degrees (360/3)
    // A: 0-120, B: 120-240, C: 240-360
    // We want the pointer (at top) to land in the middle of the winner's section
    // A center: 60°, B center: 180°, C center: 300°
    const sectionCenters = [60, 180, 300]; // degrees for A, B, C
  ...
  return (
    <Card>
      <CardHeader>
        <h2 className="text-3xl font-bold text-center text-gray-800">Three-Way Tie!</h2>
